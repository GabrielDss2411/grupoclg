...
// slug, from().upsert().select().single(), from().update().eq().select().single()).
function makeSupabaseMock({
  user = { id: 'user-1', email: 'grupoclg@codesants.com.br' },
  slugCheckResponses = [],
  upsertResult = { data: { id: 'id-1', slug: 'slug-gerado' }, error: null },
  updateResult = { data: { id: 'id-1', slug: 'slug-gerado' }, error: null },
} = {}) {
  const calls = { upsert: [], update: [], eqSlug: [], updateEq: [] };
...
describe('app/admin/actions.js -- slug (AC P1-B/4, P1-B/6)', () => {
  it('slug colidindo na criação ganha sufixo -2', async () => {
    const { supabase, calls } = makeSupabaseMock({
      ...
      upsertResult: { data: { id: 'novo-id', slug: 'curso-de-teste-2' }, error: null },
    });
    state.supabase = supabase;
    ...
    expect(calls.upsert[0].payload.slug).toBe('curso-de-teste-2');
  });

  it('edição de curso existente (com id) preserva o slug -- não recalcula nem reenvia', async () => {
    const { supabase, calls } = makeSupabaseMock({
      upsertResult: { data: { id: 'id-existente', slug: 'slug-original' }, error: null },
    });
    state.supabase = supabase;
    ...
    expect(calls.upsert[0].payload.id).toBe('id-existente');
    expect(calls.upsert[0].payload).not.toHaveProperty('slug');
  });

  it('slug colidindo na criação de congresso ganha sufixo -2 (mesma lógica de P1-B/6)', async () => {
    const { supabase, calls } = makeSupabaseMock({
      slugCheckResponses: [{ data: { id: 'outro-id' }, error: null }, { data: null, error: null }],
      upsertResult: { data: { id: 'novo-id', slug: 'congresso-de-teste-2' }, error: null },
    });
    state.supabase = supabase;
    ...
    expect(calls.upsert[0].payload.slug).toBe('congresso-de-teste-2');
  });
});
...
describe('app/admin/actions.js -- validação de congresso (AC P1-C/2, P1-C/3)', () => {
  ...
  it('programação (prog) do congresso preserva a ordem definida no form (AC P1-C/4)', async () => {
    const { supabase, calls } = makeSupabaseMock({
      upsertResult: { data: { id: 'id-1', slug: 'congresso-de-teste' }, error: null },
    });
    state.supabase = supabase;
    ...
    expect(result.ok).toBe(true);
    expect(calls.upsert[0].payload.prog).toEqual(prog);
  });
});

describe('app/admin/actions.js -- sucesso chama revalidateTag/revalidatePath (ADM-08)', () => {
  it('salvarCurso com sucesso revalida a tag "cursos" e os paths de detalhe/home', async () => {
    const { supabase, calls } = makeSupabaseMock({
      upsertResult: { data: { id: 'id-1', slug: 'curso-de-teste' }, error: null },
    });
    state.supabase = supabase;
    ...
    expect(calls.upsert[0].payload.slug).toBe('curso-de-teste');
    expect(calls.upsert[0].payload.title).toBe('Curso de Teste');
    expect(calls.upsert[0].payload.cat).toBe('Licitações');
    expect(calls.upsert[0].payload.descricao).toBe('Descrição do curso de teste.');
    expect(calls.upsert[0].payload.poster).toBe('/assets/poster-14133.svg');
    expect(revalidateTag).toHaveBeenCalledWith('cursos');
    expect(revalidatePath).toHaveBeenCalledWith('/cursos/curso-de-teste');
    expect(revalidatePath).toHaveBeenCalledWith('/');
  });

  it('salvarCongresso com sucesso revalida a tag "congressos" e os paths de detalhe/lista', async () => {
    const { supabase, calls } = makeSupabaseMock({
      upsertResult: { data: { id: 'id-1', slug: 'congresso-de-teste' }, error: null },
    });
    state.supabase = supabase;
    ...
    expect(calls.upsert[0].payload.slug).toBe('congresso-de-teste');
    expect(calls.upsert[0].payload.nome).toBe('Congresso de Teste');
    expect(calls.upsert[0].payload.edicao).toBe('1ª edição');
    expect(calls.upsert[0].payload.datas).toBe('10 a 12 de outubro de 2026');
    expect(calls.upsert[0].payload.iso).toBe('2026-10-10');
    expect(calls.upsert[0].payload.local_chip).toBe('Rio de Janeiro/RJ');
    expect(calls.upsert[0].payload.formato).toBe('Presencial');
    expect(calls.upsert[0].payload.card_desc).toBe('Descrição do card de teste.');
...
describe('app/admin/actions.js -- falha de banco não lança (AC P1-B/7)', () => {
  it('salvarCurso: falha do mock no upsert retorna { ok:false } sem lançar', async () => {
    const { supabase, calls } = makeSupabaseMock({
      upsertResult: { data: null, error: new Error('conexão recusada') },
    });
    state.supabase = supabase;
    ...
    expect(calls.upsert.length).toBe(1); // tentou, mas não persistiu
  });

  it('salvarCongresso: falha do mock no upsert retorna { ok:false } sem lançar', async () => {
...
describe('app/admin/actions.js -- idempotência do duplo clique (AC P1-B/8)', () => {
  it('chamar salvarCurso duas vezes com o mesmo id faz upsert (update) as duas vezes -- nunca insert novo', async () => {
    // Simula duplo clique numa edição: o botão do form fica desabilitado
    // durante o envio (T19), mas a garantia de fundo é esta: upsert por id
    // não cria duas linhas mesmo se a action for chamada mais de uma vez.
    const { supabase, calls } = makeSupabaseMock({
      upsertResult: { data: { id: 'id-1', slug: 'slug-original' }, error: null },
    });
    state.supabase = supabase;
    ...
    expect(calls.upsert[0].payload.id).toBe('id-1');
    expect(calls.upsert[1].payload.id).toBe('id-1');
    expect(calls.upsert[0].payload).not.toHaveProperty('slug');
    expect(calls.upsert[1].payload).not.toHaveProperty('slug');
  });

  it('chamar salvarCongresso duas vezes com o mesmo id faz upsert (update) as duas vezes -- nunca insert novo (AC P1-C/6, regras idênticas a P1-B/8)', async () => {
    const { supabase, calls } = makeSupabaseMock({
      upsertResult: { data: { id: 'id-2', slug: 'slug-original' }, error: null },
    });
    state.supabase = supabase;
    ...
    expect(r2.ok).toBe(true);
    expect(calls.upsert.length).toBe(2);
    expect(calls.upsert[0].payload.id).toBe('id-2');
    expect(calls.upsert[1].payload.id).toBe('id-2');
    expect(calls.upsert[0].payload).not.toHaveProperty('slug');
    expect(calls.upsert[1].payload).not.toHaveProperty('slug');
  });
});
