...
export function cursoHtml(cursoOuSlug) {
  // TODO(Fase 3/T13): remover este bridge quando app/cursos/[slug]/page.jsx
  // passar a chamar cursoHtml() só com objeto do banco (lib/db.js).
  let c;
  if (typeof cursoOuSlug === 'string') {
    const curso = CURSOS[cursoOuSlug];
    if (!curso) return null;
    const { desc, ...rest } = curso;
    c = { slug: cursoOuSlug, descricao: desc, disponivel: true, ...rest };
  } else {
    c = cursoOuSlug;
  }
  if (!c) return null;
  const wa = `${WA}?text=${encodeURIComponent(`Olá, quero garantir minha vaga no curso ${escapeHtml(c.title)}.`)}`;
