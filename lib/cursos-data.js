...
// Para adicionar um curso: nova entrada aqui + card com data-href="/cursos/<slug>" no content.js.
...
export function cursoHtml(slug) {
  ...
  <div style="max-width:1160px;margin:0 auto;padding:130px 30px 40px;display:grid;grid-template-columns:1.15fr 0.85fr;gap:56px;align-items:start">
    ...
    <div>
      ...
      <div style="font-size:12px;letter-spacing:2.4px;font-weight:700;color:#A89A6E;margin-top:22px">${c.cat.toUpperCase()} · ${c.tag.toUpperCase()}</div>
      <h1 style="font-weight:600;font-size:42px;line-height:1.08;color:#0C1A57;margin:12px 0 0">${c.title}</h1>
      <p style="font-size:18px;color:#5A6180;margin:16px 0 0;line-height:1.6">${c.desc}</p>
    ...
    <div style="position:sticky;top:110px">
      <div style="border-radius:16px;overflow:hidden;aspect-ratio:380/500;box-shadow:0 20px 44px -34px rgba(10,20,66,0.45)"><img src="${c.poster}" alt="${c.title}" style="display:block;width:100%;height:100%;object-fit:cover"></div>
      <div style="border:1px solid #e5e5ea;border-radius:16px;padding:24px 26px;margin-top:18px;background:#fff">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;margin-top:16px">
          <div style="display:flex;justify-content:space-between;font-size:14.5px;color:#42496A">Inscrições até <b style="color:#0C1A57;font-weight:600">${c.inscricoes}</b></div>
          <div style="display:flex;justify-content:space-between;font-size:14.5px;color:#42496A">Início da turma <b style="color:#0C1A57;font-weight:600">${c.inicio}</b></div>
          <div style="font-size:13.5px;color:#C9A227;font-weight:600">${c.vagas}</div>
        </div>
        ...
        <a href="${wa}" target="_blank" rel="noopener" style="margin-top:16px;background:#E9C65A;color:#0A1442;border-radius:999px;padding:15px;font-weight:700;font-size:15px;display:flex;align-items:center;justify-content:center;gap:8px" data-hv="gold">Garantir vaga <span>→</span></a>
