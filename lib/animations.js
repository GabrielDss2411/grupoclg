 ...
 *
 * O GSAP é carregado sob demanda (dynamic import) para ficar fora do
 * First Load JS; se a limpeza rodar antes de ele chegar, nada é iniciado.
 */
export function initAnimations() {
  ...
  import('gsap')
    .then(({ gsap }) => { if (!cancelled && gsap) run(gsap, observers, motivoCleanups); })
    .catch(() => {});
...
function run(gsap, observers, motivoCleanups) {
