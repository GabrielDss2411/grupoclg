...
// --- Carrossel "turmas abertas" (home) e fileiras do catálogo (/cursos) ---
// T10: extraídos das strings estáticas originais para builders parametrizados
// (turmasHtml/catalogoHtml), que recebem a lista de cursos em shape de coluna
// do banco (ver lib/db.js). Fase 3 (T12): `homeHtml(cursos)`/`cursosPageHtml(cursos)`
// abaixo recebem a lista de cursos disponíveis vinda do banco (via
// getCursos() em lib/db.js) como parâmetro, no lugar do seed fixo -- o
// restante do markup permanece string estática (AD-007). A página de
// congressos (/congressos) chama `congressosPageHtml(congressos)` direto de
// lib/congressos-data.js -- não há mais um re-export "congressos" aqui.

// Metadados de apresentação do carrossel de turmas que não têm coluna própria
// no banco (rótulo do badge de categoria, gradiente do card, título curto) --
// preservam exatamente a copy/estilo do site original (AD-007). Um curso fora
// desta tabela (ex.: incluído futuramente pelo painel) usa um fallback
// derivado dos próprios campos do banco (cat, title) e um gradiente padrão,
// sem lançar erro.
const TURMA_VITRINE = {
  'lei-14133': { rotulo: 'LICITAÇÕES', gradiente: '#13275F,#0A1236' },
  'pregao': { rotulo: 'LICITAÇÕES', gradiente: '#12285E,#0A1236' },
  'fiscalizacao': { rotulo: 'CONTRATOS', gradiente: '#1A2C66,#0A1236' },
  'pareceres': { rotulo: 'JURÍDICO', gradiente: '#1E2A66,#0A1236', tituloCurto: 'Assessoria Jurídica e Pareceres' },
  'fornecedores': { rotulo: 'WORKSHOP', gradiente: '#163A6B,#0A1236' },
  'mrosc': { rotulo: 'TERCEIRO SETOR', gradiente: '#15296B,#0A1236' },
};
function turmaCard(curso, hvWrapper, hvBotao) {
  const meta = TURMA_VITRINE[curso.slug] || {};
  const rotulo = meta.rotulo || curso.cat.toUpperCase();
  const titulo = meta.tituloCurto || curso.title;
  // Card limpo (fundo branco, cantos 20px). O 'pointer-events' no style isenta
  // este card da regra global de cantos crispos (base.css) -- mesmo padrão da navbar.
  return `      <!-- CARD -->
      <div style="pointer-events:auto;flex:0 0 300px;scroll-snap-align:start;background:#fff;border:1px solid #e5e5ea;border-radius:20px;overflow:hidden;display:flex;flex-direction:column;transition:box-shadow .25s ease, border-color .25s ease" data-hv="hv${hvWrapper}">
        <div style="padding:22px 22px 0">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
            <div style="font-size:11px;letter-spacing:1.2px;font-weight:700;color:#C9A227">${escapeHtml(rotulo)}</div>
            <div style="display:inline-flex;align-items:center;gap:7px;background:rgba(91,214,160,0.14);border:1px solid rgba(91,214,160,0.42);border-radius:999px;padding:5px 10px;font-size:11px;font-weight:600;color:#0C1A57"><span style="width:7px;height:7px;border-radius:50%;background:#5BD6A0;display:inline-block"></span>Vagas abertas</div>
          </div>
          <h3 style="font-size:19px;font-weight:600;color:#0C1A57;margin:16px 0 0;line-height:1.3">${escapeHtml(titulo)}</h3>
        </div>
        <div style="padding:18px 22px 22px;flex:1;display:flex;flex-direction:column;gap:12px">
          <div style="height:1px;background:#e5e5ea"></div>
          <div style="display:flex;align-items:center;gap:10px;font-size:14px;color:#42496A"><span style="color:#C9A227"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><rect x="3.5" y="5" width="17" height="15" rx="2"/><path d="M3.5 9h17M8 3.5v3M16 3.5v3"/></svg></span>Inscrições até <b style="color:#0C1A57;font-weight:600">${escapeHtml(curso.inscricoes)}</b></div>
          <div style="display:flex;align-items:center;gap:10px;font-size:14px;color:#42496A"><span style="color:#C9A227"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.7"><circle cx="12" cy="12" r="8.5"/><path d="M10 8.3l6 3.7-6 3.7z" fill="currentColor" stroke="none"/></svg></span>Início <b style="color:#0C1A57;font-weight:600">${escapeHtml(curso.inicio)}</b> · ${escapeHtml(curso.horas)}</div>
          <div style="display:flex;align-items:center;gap:10px;font-size:13px;color:#C9A227;font-weight:600">${escapeHtml(curso.vagas)}</div>
          <button data-href="/cursos/${curso.slug}" style="margin-top:auto;background:#0C1A57;color:#fff;border:0;border-radius:999px;padding:12px;font-weight:700;font-size:14px;cursor:pointer;width:100%;display:inline-flex;align-items:center;justify-content:center;gap:8px" data-hv="hv${hvBotao}">Garantir vaga <span>→</span></button>
        </div>
      </div>`;
}

// Carrossel de turmas abertas (home). Edge case: lista vazia produz um
// carrossel sem cards (sem lançar erro) -- não há estado vazio dedicado na
// spec para este carrossel especificamente (apenas para a agenda de
// congressos e para fileiras de catálogo).
export function turmasHtml(cursos) {
  let hv = 7;
  const cards = cursos.map((c) => {
    const card = turmaCard(c, hv, hv + 1);
    hv += 2;
    return card;
  });
  return `  <!-- TURMAS COM INSCRIÇÕES ABERTAS (carrossel) -->
...
function pluralCursos(n) {
  return n === 1 ? '1 curso disponível' : `${n} cursos disponíveis`;
}

const CATALOGO_ICONE_LICITACOES = '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3.2v16.3"/><path d="M7.8 19.5h8.4"/><path d="M4.5 7.3h15"/><path d="M4.5 7.3 2.3 12.1M4.5 7.3 6.7 12.1"/><path d="M2.3 12.1a2.2 1.9 0 0 0 4.4 0"/><path d="M19.5 7.3 17.3 12.1M19.5 7.3 21.7 12.1"/><path d="M17.3 12.1a2.2 1.9 0 0 0 4.4 0"/></svg>';
const CATALOGO_ICONE_CONTRATOS = '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M7 3.5h7l3.5 3.5V20.5H7z"/><path d="M14 3.5V7h3.5"/><path d="M9.3 13.6l1.6 1.6 3.4-3.4"/></svg>';
const CATALOGO_ICONE_TERCEIRO_SETOR = '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8" r="3"/><path d="M3.6 19.5c0-3 2.4-5.2 5.4-5.2s5.4 2.2 5.4 5.2"/><circle cx="17.2" cy="9" r="2.3"/><path d="M15.6 14.6c2.7-.1 5 1.8 5 4.9"/></svg>';
// Ícone padrão para categoria não mapeada em CATALOGO_FILEIRAS (edge case da spec).
const CATALOGO_ICONE_PADRAO = CATALOGO_ICONE_LICITACOES;

// Fileiras do catálogo: cada fileira agrupa 1+ valores de `cat` sob um mesmo
// título/ícone -- o catálogo original reúne "Fiscalização" e "Contratos" numa
// única fileira "Contratos e Pareceres", reproduzido aqui como já era (AD-007).
// Um valor de `cat` fora deste mapa ganha fileira própria, título = o próprio
// valor de `cat` e o ícone padrão acima (edge case: categoria nova).
const CATALOGO_FILEIRAS = [
  { titulo: 'Licitações e Pregão', icone: CATALOGO_ICONE_LICITACOES, cats: ['Licitações'] },
  { titulo: 'Contratos e Pareceres', icone: CATALOGO_ICONE_CONTRATOS, cats: ['Fiscalização', 'Contratos'] },
  { titulo: 'Terceiro Setor', icone: CATALOGO_ICONE_TERCEIRO_SETOR, cats: ['Terceiro Setor'] },
];

function catalogoCard(curso, hv) {
  return `        <div data-href="/cursos/${curso.slug}" style="cursor:pointer;flex:0 0 300px;scroll-snap-align:start">
          <div style="pointer-events:auto;border-radius:20px;overflow:hidden;aspect-ratio:380/500;box-shadow:0 20px 44px -34px rgba(10,20,66,0.45);transition:transform .25s ease, box-shadow .25s ease" data-hv="hv${hv}"><img src="${curso.poster}" alt="${escapeHtml(curso.title)}" style="display:block;width:100%;height:100%;object-fit:cover"></div>
          <div style="display:flex;align-items:center;justify-content:space-between;margin-top:13px"><span style="font-size:13px;color:#8A90A6">${escapeHtml(curso.modulos)} · ${escapeHtml(curso.horas)}</span><span style="font-size:12.5px;font-weight:600;color:#C9A227">Ver programa →</span></div>
        </div>`;
}

// Fileiras do catálogo de cursos (/cursos). Edge cases da spec: uma fileira
// sem nenhum curso disponível é omitida (sem cabeçalho vazio); uma categoria
// fora do mapa acima ganha fileira própria com ícone padrão, sem erro.
export function catalogoHtml(cursos) {
  let hv = 22;
  const linhas = [];
  const usados = new Set();

  function renderFileira(titulo, icone, lista, primeira) {
    const hvPrev = hv++;
    const hvNext = hv++;
    const cards = lista.map((c) => {
      const card = catalogoCard(c, hv);
      hv += 1;
      return card;
    });
    return `    <!-- ROW: ${titulo} -->
    <div data-row${primeira ? '' : ' style="margin-top:64px"'}>
      <div style="display:flex;align-items:center;justify-content:space-between;gap:16px">
        <div style="display:flex;align-items:center;gap:12px">
          <span style="color:#C9A227">${icone}</span>
          <span style="font-size:21px;font-weight:600;color:#0C1A57;letter-spacing:-0.01em">${escapeHtml(titulo)}</span>
        </div>
        <div style="display:flex;align-items:center;gap:14px">
          <span style="font-size:14.5px;color:#8A90A6">${pluralCursos(lista.length)}</span>
          <div style="display:flex;gap:8px">
            <button data-slide="prev" aria-label="Anterior" style="width:38px;height:38px;border-radius:50%;background:#fff;border:1px solid #e5e5ea;color:#0C1A57;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:20px;line-height:1;padding-bottom:2px" data-hv="hv${hvPrev}">‹</button>
            <button data-slide="next" aria-label="Próximo" style="width:38px;height:38px;border-radius:50%;background:#fff;border:1px solid #e5e5ea;color:#0C1A57;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:20px;line-height:1;padding-bottom:2px" data-hv="hv${hvNext}">›</button>
          </div>
        </div>
      </div>
...
${cards.join('\n')}
  ...
  for (const fileira of CATALOGO_FILEIRAS) {
    const doFileira = cursos.filter((c) => fileira.cats.includes(c.cat));
    doFileira.forEach((c) => usados.add(c.cat));
    if (doFileira.length === 0) continue;
    linhas.push(renderFileira(fileira.titulo, fileira.icone, doFileira, linhas.length === 0));
  }
  const catsNovas = [...new Set(cursos.map((c) => c.cat).filter((cat) => !usados.has(cat)))];
  for (const cat of catsNovas) {
    const doCat = cursos.filter((c) => c.cat === cat);
    linhas.push(renderFileira(cat, CATALOGO_ICONE_PADRAO, doCat, linhas.length === 0));
  }

  return `  <!-- CATÁLOGO EM LINHAS (cards pôster) -->
...
export function homeHtml(cursos) {
  return `<div>
...
export function cursosPageHtml(cursos) {
  return `<div>
...
export const navbar = `<div style="position:fixed;top:0;left:0;right:0;z-index:60;padding:18px 24px;display:flex;justify-content:center;pointer-events:none">
  <div style="pointer-events:auto;width:100%;max-width:1200px;margin:0 auto;padding:12px 16px 12px 20px;display:flex;align-items:center;gap:36px;background:rgba(10,16,40,0.72);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,0.1);border-radius:20px;box-shadow:0 14px 40px -26px rgba(0,0,0,0.5)">
    <button data-nav="home" style="display:flex;align-items:center;background:none;border:0;cursor:pointer;padding:0">
      <img src="/assets/logo-grupoclg.png" alt="Grupo CLG" style="height:46px;width:auto;display:block" />
    </button>
    <nav style="display:flex;gap:4px;margin-left:8px">
      ...
      <button data-nav="artigos" data-navkey="artigos" style="position:relative;background:none;border:0;cursor:pointer;font-size:15px;font-weight:500;color:rgba(255,255,255,0.72);padding:9px 16px;border-radius:999px" data-hv="hv51"><span style="position:relative">Artigos</span></button>
...
export const footer = `<div style="background:#0A1442;color:#fff">
  <div style="max-width:1200px;margin:0 auto;padding:66px 30px 40px;display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr;gap:40px">
    <div>
      <img src="/assets/logo-grupoclg.png" alt="Grupo CLG" style="height:52px;width:auto;display:block" />
    ...
    <div>
      ...
      <div style="display:flex;flex-direction:column;gap:12px;margin-top:18px">
        ...
        <button data-nav="artigos" style="background:none;border:0;color:#B8C0DA;font-size:15px;text-align:left;cursor:pointer;padding:0" data-hv="hv43">Artigos</button>
    ...
    <div>
      ...
      <div style="display:flex;flex-direction:column;gap:12px;margin-top:18px">
        <button data-nav="artigos" style="background:none;border:0;color:#B8C0DA;font-size:15px;text-align:left;cursor:pointer;padding:0" data-hv="hv45">Artigos e Notícias</button>
