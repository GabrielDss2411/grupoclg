...
// do banco (ver lib/db.js). `home`/`cursos` abaixo mantêm o nome exportado
// atual, computados a partir dessas funções + o seed de CURSOS -- mesmo
// padrão de bridge documentado em T8/T9 (lib/cursos-data.js,
// lib/congressos-data.js), substituído na Fase 3 quando as páginas passarem a
// chamar essas funções só com dados reais do banco.
...
const TURMA_GRADIENTE_PADRAO = '#13275F,#0A1236';
...
function turmaCard(curso, hvWrapper, hvBotao) {
  ...
  const gradiente = meta.gradiente || TURMA_GRADIENTE_PADRAO;
  const titulo = meta.tituloCurto || curso.title;
  return `      <!-- CARD -->
      <div style="flex:0 0 300px;scroll-snap-align:start;background:#fff;border:1px solid #e5e5ea;border-radius:16px;overflow:hidden;display:flex;flex-direction:column;transition:box-shadow .25s ease, border-color .25s ease" data-hv="hv${hvWrapper}">
        <div style="position:relative;padding:22px 22px 20px;background:radial-gradient(300px 200px at 92% 0%, rgba(41,66,150,0.5), transparent 60%), linear-gradient(150deg,${gradiente});color:#fff">
          ...
          <div style="font-size:11px;letter-spacing:1.2px;font-weight:700;color:#E9C65A">${escapeHtml(rotulo)}</div>
          <h3 style="font-size:19px;font-weight:600;color:#fff;margin:28px 0 0;line-height:1.25">${escapeHtml(titulo)}</h3>
        </div>
...
function catalogoCard(curso, hv) {
  return `        <div data-href="/cursos/${curso.slug}" style="cursor:pointer;flex:0 0 300px;scroll-snap-align:start">
          <div style="border-radius:16px;overflow:hidden;aspect-ratio:380/500;box-shadow:0 20px 44px -34px rgba(10,20,66,0.45);transition:transform .25s ease, box-shadow .25s ease" data-hv="hv${hv}"><img src="${curso.poster}" alt="${escapeHtml(curso.title)}" style="display:block;width:100%;height:100%;object-fit:cover"></div>
...
// Ordem curada do carrossel de turmas na home -- preserva a ordem visual
// atual (distinta da ordem de declaração de CURSOS e da futura ordenação por
// created_at em lib/db.js; ver AD-007, equivalência é só com o seed atual).
const ORDEM_TURMAS = ['lei-14133', 'pregao', 'fiscalizacao', 'pareceres', 'fornecedores', 'mrosc'];
const CURSOS_TURMAS = ORDEM_TURMAS.map(toColumnShape);
const CURSOS_CATALOGO = Object.keys(CURSOS).map(toColumnShape);
