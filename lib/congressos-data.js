 ...
 * Tema escuro (identidade navy + dourado do Grupo CLG), inspirado em um layout
 * de "event schedule": cabeçalho, abas por evento, cards de agenda com bloco de
 * data institucional (sem fotos de evento, por autoridade) e faixa em gradiente.
 */
...
function agendaTab(c, ativo) {
  const base = 'flex:0 0 auto;display:flex;flex-direction:column;gap:2px;text-decoration:none;border-radius:14px;padding:10px 18px;min-width:118px';
  const estilo = ativo
    ? `${base};background:linear-gradient(120deg,#E9C65A,#C9A227);color:#0A1442`
    : `${base};background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.12);color:#B7C0DC`;
  const corMes = ativo ? '#0A1442' : '#8E9AC0';
  return `<a href="#cong-${c.slug}" style="${estilo}"><span style="font-size:14px;font-weight:700">${escapeHtml(c.local_chip)}</span><span style="font-size:11.5px;letter-spacing:1px;font-weight:700;opacity:0.85;color:${corMes}">${escapeHtml(c.card_mes)}</span></a>`;
}

function agendaCard(c) {
  const aberto = /abert/i.test(c.status || '');
  const corStatus = aberto ? '#5BD6A0' : '#8E9AC0';
  return `<div id="cong-${c.slug}" style="scroll-margin-top:104px;border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:22px 26px;display:grid;grid-template-columns:132px minmax(0,1fr) 210px 176px;gap:22px;align-items:center;background:rgba(255,255,255,0.035)" data-hv="card">
        <div style="text-align:center;border-radius:12px;padding:16px 10px;background:linear-gradient(150deg,rgba(233,198,90,0.16),rgba(201,162,39,0.05));border:1px solid rgba(233,198,90,0.24)">
          <div style="font-size:30px;font-weight:700;color:#fff;letter-spacing:-0.02em;line-height:1">${escapeHtml(c.card_dias)}</div>
          <div style="font-size:11px;letter-spacing:1.4px;font-weight:700;color:#E9C65A;margin-top:6px">${escapeHtml(c.card_mes)}</div>
          <div style="display:inline-block;margin-top:10px;font-size:11px;font-weight:700;color:#E9C65A;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.14);border-radius:999px;padding:3px 10px">${escapeHtml(c.edicao)}</div>
        </div>
        <div>
          <div style="font-size:11.5px;letter-spacing:1.6px;font-weight:700;color:#8E9AC0">${escapeHtml(c.card_local)}</div>
          <h3 style="font-size:21px;color:#fff;margin:8px 0 0;line-height:1.2">${escapeHtml(c.nome)}</h3>
          <p style="font-size:14.5px;color:#9AA3C2;line-height:1.55;margin:8px 0 0;max-width:520px">${escapeHtml(c.card_desc)}</p>
        </div>
        <div style="display:flex;flex-direction:column;gap:11px">
          <span style="display:inline-flex;align-items:center;gap:9px;font-size:13.5px;color:#B7C0DC">${icoRelogio}${escapeHtml(c.datas)}</span>
          <span style="display:inline-flex;align-items:center;gap:9px;font-size:13.5px;color:#B7C0DC">${icoLocal}${escapeHtml(c.local_chip)} · ${escapeHtml(c.formato)}</span>
        </div>
...
const agendaVazia = `<div style="border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:48px 30px;text-align:center;background:rgba(255,255,255,0.035);margin-top:26px">
      <p style="font-size:17px;color:#9AA3C2;margin:0">Novos eventos em breve.</p>
    </div>`;

export function congressosPageHtml(lista) {
  const temEventos = lista.length > 0;
  const tabs = temEventos
    ? `<div class="hide-scroll" style="display:flex;gap:10px;overflow-x:auto;max-width:100%">${lista.map((c, i) => agendaTab(c, i === 0)).join('')}</div>`
    : '';
  const cards = temEventos
    ? `<div style="display:flex;flex-direction:column;gap:16px;margin-top:26px">
      ${lista.map((c) => agendaCard(c)).join('\n\n      ')}
    </div>`
    : agendaVazia;

  return `<div style="background:linear-gradient(180deg,#070E33,#0A1236 46%,#070E33)">

  <!-- HEADER -->
  <div style="position:relative;overflow:hidden;background:radial-gradient(1200px 620px at 74% 30%, rgba(41,66,150,0.5), transparent 58%), linear-gradient(158deg,#0C1A57,#070E33)">
    <div style="max-width:1200px;margin:0 auto;padding:150px 30px 64px;text-align:center">
      <div style="font-size:12px;letter-spacing:2.4px;font-weight:700;color:#E9C65A">CONGRESSOS E EVENTOS</div>
      <h1 style="font-weight:600;font-size:48px;line-height:1.06;color:#fff;margin:16px auto 0;max-width:860px">Os grandes encontros de quem faz a <span style="color:#E9C65A">contratação pública</span> acontecer.</h1>
      <p style="font-size:18px;color:#B7C0DC;margin:18px auto 0;max-width:660px;line-height:1.55">Dias de imersão com professores de referência, painéis sobre a jurisprudência mais recente e networking com profissionais de todo o Brasil.</p>
      <div style="display:inline-flex;align-items:center;gap:9px;font-size:13px;font-weight:600;color:#9AA3C2;margin-top:22px">Início <span style="color:#C9A227">›</span> <span style="color:#fff">Congressos</span></div>
    </div>
  </div>
  ...
  <div style="max-width:1200px;margin:0 auto;padding:66px 30px 20px">
    <div style="display:flex;align-items:flex-end;justify-content:space-between;gap:24px;flex-wrap:wrap">
      <div>
        ...
        <h2 style="font-size:34px;color:#fff;margin:10px 0 0">Siga a agenda dos congressos</h2>
      </div>
      ${tabs}
    </div>
    ${cards}
  ...
  <div style="max-width:1200px;margin:0 auto;padding:44px 30px 20px">
    <div style="font-size:12px;letter-spacing:2.4px;font-weight:700;color:#C9A227">POR QUE PARTICIPAR</div>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:18px;margin-top:20px">
      <div style="border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:24px 26px;background:rgba(255,255,255,0.035)">
        <div style="font-size:16px;font-weight:600;color:#fff">Conteúdo aplicável</div>
        <p style="font-size:14.5px;color:#9AA3C2;line-height:1.55;margin:8px 0 0">Painéis construídos a partir de casos reais e da jurisprudência do TCU.</p>
      </div>
      <div style="border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:24px 26px;background:rgba(255,255,255,0.035)">
        <div style="font-size:16px;font-weight:600;color:#fff">Professores atuantes</div>
        <p style="font-size:14.5px;color:#9AA3C2;line-height:1.55;margin:8px 0 0">Quem palestra é quem atua diariamente na Administração Pública.</p>
      </div>
      <div style="border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:24px 26px;background:rgba(255,255,255,0.035)">
        <div style="font-size:16px;font-weight:600;color:#fff">Networking nacional</div>
        <p style="font-size:14.5px;color:#9AA3C2;line-height:1.55;margin:8px 0 0">Troque experiências com servidores e profissionais de todo o país.</p>
      </div>
      <div style="border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:24px 26px;background:rgba(255,255,255,0.035)">
        <div style="font-size:16px;font-weight:600;color:#fff">Certificado</div>
        <p style="font-size:14.5px;color:#9AA3C2;line-height:1.55;margin:8px 0 0">Certificado de participação individual para sua progressão funcional.</p>
      </div>
    </div>
  </div>
...
export function congressoHtml(c) {
  if (!c) return null;
  const wa = `${WA}?text=${encodeURIComponent(`Olá, quero garantir minha vaga no evento ${escapeHtml(c.nome)} (${escapeHtml(c.datas)}).`)}`;
