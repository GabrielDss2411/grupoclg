...
function agendaCard(c) {
  return `<div style="border:1px solid #e5e5ea;border-radius:16px;padding:28px 30px;display:grid;grid-template-columns:190px 1fr auto;gap:28px;align-items:center;background:#fff;transition:box-shadow .25s ease" data-hv="card">
        <div>
          <div style="font-size:34px;font-weight:600;color:#0C1A57;letter-spacing:-0.02em;line-height:1">${escapeHtml(c.card_dias)}</div>
          <div style="font-size:13px;letter-spacing:1.8px;font-weight:700;color:#C9A227;margin-top:6px">${escapeHtml(c.card_mes)}</div>
          <div style="display:inline-block;margin-top:12px;font-size:12px;font-weight:700;color:#0C1A57;background:#f5f5f7;border:1px solid #e5e5ea;border-radius:999px;padding:5px 12px">${escapeHtml(c.edicao)}</div>
        </div>
        <div>
          <div style="font-size:12px;letter-spacing:1.8px;font-weight:700;color:#8A90A6">${escapeHtml(c.card_local)}</div>
          <h3 style="font-size:23px;color:#0C1A57;margin:8px 0 0">${escapeHtml(c.nome)}</h3>
          <p style="font-size:15.5px;color:#5A6180;line-height:1.55;margin:8px 0 0;max-width:560px">${escapeHtml(c.card_desc)}</p>
        </div>
        <div style="display:flex;flex-direction:column;gap:10px;align-items:stretch">
          <button data-href="/congressos/${c.slug}" style="background:#0C1A57;color:#fff;border:0;cursor:pointer;border-radius:999px;padding:13px 26px;font-weight:700;font-size:14.5px;text-align:center;display:inline-flex;align-items:center;justify-content:center;gap:8px" data-hv="navy">Saiba mais <span>→</span></button>
          <span style="font-size:12.5px;color:#8A90A6;text-align:center">${escapeHtml(c.status)}</span>
        </div>
      </div>`;
}

// Edge case da spec: agenda sem nenhum congresso disponível exibe estado vazio amigável.
const agendaVazia = `<div style="max-width:1200px;margin:0 auto;padding:48px 30px 20px">
    <div style="font-size:12px;letter-spacing:2.4px;font-weight:700;color:#A89A6E">AGENDA 2026</div>
    <div style="border:1px solid #e5e5ea;border-radius:16px;padding:48px 30px;text-align:center;margin-top:20px">
      <p style="font-size:17px;color:#5A6180;margin:0">Novos eventos em breve.</p>
    </div>
  </div>`;

export function congressosPageHtml(lista) {
  const agenda = lista.length === 0
    ? agendaVazia
...
// Export estrutural mantido para o site atual (lib/content.js) e para os guard
// rails, computado a partir da mesma função parametrizada acima com o seed de
// CONGRESSOS -- ver AD-007. Passa a ser substituído em app/congressos/page.jsx
// (Fase 3) por uma chamada direta a congressosPageHtml(dbCongressos).
export const congressosPage = congressosPageHtml(
  Object.entries(CONGRESSOS).map(([slug, c]) => ({
    slug,
    nome: c.nome,
    edicao: c.edicao,
    tagline: c.tagline,
...
export function congressoHtml(congressoOuSlug) {
  // TODO(Fase 3/T13): remover este bridge quando app/congressos/[slug]/page.jsx
  // passar a chamar congressoHtml() só com objeto do banco (lib/db.js).
  let c;
  if (typeof congressoOuSlug === 'string') {
    const congresso = CONGRESSOS[congressoOuSlug];
    if (!congresso) return null;
    c = {
      slug: congressoOuSlug,
      nome: congresso.nome,
      edicao: congresso.edicao,
      tagline: congresso.tagline,
      datas: congresso.datas,
      iso: congresso.iso,
      card_dias: congresso.cardDias,
      card_mes: congresso.cardMes,
      card_local: congresso.cardLocal,
      card_desc: congresso.cardDesc,
      status: congresso.status,
      local_chip: congresso.localChip,
      formato: congresso.formato,
      sobre: congresso.sobre,
      stats: congresso.stats,
      local_nome: congresso.localNome,
      local_desc: congresso.localDesc,
      prog: congresso.prog,
      disponivel: true,
    };
  } else {
