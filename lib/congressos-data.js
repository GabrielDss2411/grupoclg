...
// CONGRESSOS abaixo é usado só como seed/fixture (ver scripts/seed e testes) --
// congressoHtml()/congressosPageHtml() recebem objetos no formato das colunas do
// banco (ver lib/db.js).
 ...
 * Mesma identidade clara do resto do site (fundo branco, navy + dourado),
 * inspirado em um layout de "event schedule": cabeçalho, abas por evento,
 * cards de agenda com bloco de data institucional (sem fotos de evento, por
 * autoridade) e faixa final em gradiente navy (mesmo padrão de CTA usado em
 * cursoHtml/congressoHtml).
 */

const icoRelogio = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#C9A227" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>`;
const icoLocal = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#C9A227" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><path d="M12 21s7-6.3 7-11a7 7 0 1 0-14 0c0 4.7 7 11 7 11z"/><circle cx="12" cy="10" r="2.5"/></svg>`;

// Nomes de mês fixos em português -- o mês do calendário é derivado de
// c.iso (data real, formato "AAAA-MM-DD") lendo os dígitos diretamente,
// nunca via Intl/toLocaleDateString: assim o mês nunca muda com a
// localidade do navegador nem do ambiente onde a página é renderizada.
const MESES_PT = ['janeiro', 'fevereiro', 'março', 'abril', 'maio', 'junho', 'julho', 'agosto', 'setembro', 'outubro', 'novembro', 'dezembro'];

function mesInfoDoCongresso(c) {
  const idx = Number(String(c.iso).slice(5, 7)) - 1;
  const nome = MESES_PT[idx] || '';
  const slug = nome.normalize('NFD').replace(/\p{Diacritic}/gu, '') || 'sem-data';
  const label = nome ? nome.charAt(0).toUpperCase() + nome.slice(1) : 'Data a definir';
  return { slug, label };
}

// Calendário por mês (só o nome do mês, sem local/formato -- essas
// informações continuam no corpo do card), com uma aba extra "Todos"
// (slug reservado 'todos', mostra a agenda inteira): clicar filtra quais
// eventos aparecem (delegado por useSiteHandlers via
// [data-mes-filtro]/[data-mes] em components/handlers.js). "Todos" começa
// ativo -- o estado inicial já vem certo do servidor (todos os cards com
// display:grid), sem flash de conteúdo não filtrado no primeiro paint.
function mesTab(label, slug, ativo) {
  const base = 'flex:0 0 auto;border-radius:999px;padding:12px 22px;font-size:14.5px;font-weight:700;cursor:pointer;border:1px solid #e5e5ea;background:#fff;color:#42496A';
  return `<button type="button" data-mes-filtro="${slug}" class="cong-mes-tab${ativo ? ' cong-mes-ativo' : ''}" style="${base}">${escapeHtml(label)}</button>`;
}

function agendaCard(c, mesSlug, ativo) {
  const aberto = /abert/i.test(c.status || '');
  const corStatus = aberto ? '#5BD6A0' : '#8A90A6';
  return `<div id="cong-${c.slug}" data-mes="${mesSlug}" style="scroll-margin-top:104px;border:1px solid #e5e5ea;border-radius:16px;padding:22px 26px;display:${ativo ? 'grid' : 'none'};grid-template-columns:132px minmax(0,1fr) 210px 176px;gap:22px;align-items:center;background:#fff;box-shadow:0 20px 44px -34px rgba(10,20,66,0.45)" data-hv="card">
        <div style="text-align:center;border-radius:12px;padding:16px 10px;background:linear-gradient(150deg,rgba(233,198,90,0.16),rgba(201,162,39,0.05));border:1px solid rgba(233,198,90,0.24)">
          <div style="font-size:30px;font-weight:700;color:#0C1A57;letter-spacing:-0.02em;line-height:1">${escapeHtml(c.card_dias)}</div>
          <div style="font-size:11px;letter-spacing:1.4px;font-weight:700;color:#C9A227;margin-top:6px">${escapeHtml(c.card_mes)}</div>
          <div style="display:inline-block;margin-top:10px;font-size:11px;font-weight:700;color:#C9A227;background:#fff;border:1px solid rgba(233,198,90,0.4);border-radius:999px;padding:3px 10px">${escapeHtml(c.edicao)}</div>
        </div>
        <div>
          <div style="font-size:11.5px;letter-spacing:1.6px;font-weight:700;color:#A89A6E">${escapeHtml(c.card_local)}</div>
          <h3 style="font-size:21px;color:#0C1A57;margin:8px 0 0;line-height:1.2">${escapeHtml(c.nome)}</h3>
          <p style="font-size:14.5px;color:#5A6180;line-height:1.55;margin:8px 0 0;max-width:520px">${escapeHtml(c.card_desc)}</p>
        </div>
        <div style="display:flex;flex-direction:column;gap:11px">
          <span style="display:inline-flex;align-items:center;gap:9px;font-size:13.5px;color:#42496A">${icoRelogio}${escapeHtml(c.datas)}</span>
          <span style="display:inline-flex;align-items:center;gap:9px;font-size:13.5px;color:#42496A">${icoLocal}${escapeHtml(c.local_chip)} · ${escapeHtml(c.formato)}</span>
        </div>
        <div style="display:flex;flex-direction:column;gap:10px;align-items:stretch">
          <button data-href="/congressos/${c.slug}" style="background:#E9C65A;color:#0A1442;border:0;cursor:pointer;border-radius:999px;padding:13px 22px;font-weight:700;font-size:14px;text-align:center;display:inline-flex;align-items:center;justify-content:center;gap:7px" data-hv="gold">Saiba mais <span>→</span></button>
          <span style="display:inline-flex;align-items:center;justify-content:center;gap:7px;font-size:12.5px;color:${corStatus}"><span style="width:7px;height:7px;border-radius:50%;background:${corStatus}"></span>${escapeHtml(c.status)}</span>
        </div>
      </div>`;
}

// Edge case da spec: agenda sem nenhum congresso disponível exibe estado vazio amigável.
const agendaVazia = `<div style="border:1px solid #e5e5ea;border-radius:16px;padding:48px 30px;text-align:center;background:#f5f5f7;margin-top:26px">
      <p style="font-size:17px;color:#5A6180;margin:0">Novos eventos em breve.</p>
    </div>`;

export function congressosPageHtml(lista) {
  const temEventos = lista.length > 0;

  // Ordena por data e resolve o mês de cada congresso uma única vez.
  const itens = [...lista]
    .sort((a, b) => new Date(a.iso) - new Date(b.iso))
    .map((c) => ({ c, mes: mesInfoDoCongresso(c) }));

  const meses = [];
  const mesesVistos = new Set();
  for (const { mes } of itens) {
    if (!mesesVistos.has(mes.slug)) {
      mesesVistos.add(mes.slug);
      meses.push(mes);
    }
  }
  // "Todos" começa ativo (mostra a agenda inteira); os meses filtram a partir daí.
  const mesAtivo = 'todos';

  const tabs = temEventos
    ? `<div class="hide-scroll" style="display:flex;gap:10px;overflow-x:auto;max-width:100%">${mesTab('Todos', 'todos', true)}${meses.map((m) => mesTab(m.label, m.slug, false)).join('')}</div>`
    : '';
  const cards = temEventos
    ? `<div style="display:flex;flex-direction:column;gap:16px;margin-top:26px">
      ${itens.map(({ c, mes }) => agendaCard(c, mes.slug, mesAtivo === 'todos' || mes.slug === mesAtivo)).join('\n\n      ')}
    </div>`
    : agendaVazia;
  ...
  <!-- HEADER -->
  <div style="max-width:1200px;margin:0 auto;padding:130px 30px 10px;text-align:center">
    <div style="font-size:12px;letter-spacing:2.4px;font-weight:700;color:#A89A6E">CONGRESSOS E EVENTOS</div>
    <h1 style="font-weight:600;font-size:48px;line-height:1.06;color:#0C1A57;margin:14px auto 0;max-width:860px">Os grandes encontros de quem faz a <span style="color:#C9A227">contratação pública</span> acontecer.</h1>
    <p style="font-size:18px;color:#5A6180;margin:18px auto 0;max-width:660px;line-height:1.55">Dias de imersão com professores de referência, painéis sobre a jurisprudência mais recente e networking com profissionais de todo o Brasil.</p>
    <div style="display:inline-flex;align-items:center;gap:9px;font-size:13px;font-weight:600;color:#8A90A6;margin-top:22px">Início <span style="color:#C9A227">›</span> <span style="color:#0C1A57">Congressos</span></div>
  </div>

  <!-- AGENDA -->
  <div style="max-width:1200px;margin:0 auto;padding:66px 30px 20px">
    <div style="display:flex;align-items:flex-end;justify-content:space-between;gap:24px;flex-wrap:wrap">
      <div>
        <div style="font-size:12px;letter-spacing:2.4px;font-weight:700;color:#C9A227">AGENDA 2026</div>
        <h2 style="font-size:34px;color:#0C1A57;margin:10px 0 0">Siga a agenda dos congressos</h2>
      </div>
      ${tabs}
    </div>
    ${cards}
  ...
  <div style="max-width:1200px;margin:0 auto;padding:44px 30px 20px">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:18px;margin-top:20px">
      <div style="border:1px solid #e5e5ea;border-radius:16px;padding:24px 26px;background:#fff">
        <div style="font-size:16px;font-weight:600;color:#0C1A57">Conteúdo aplicável</div>
        <p style="font-size:14.5px;color:#5A6180;line-height:1.55;margin:8px 0 0">Painéis construídos a partir de casos reais e da jurisprudência do TCU.</p>
      </div>
      <div style="border:1px solid #e5e5ea;border-radius:16px;padding:24px 26px;background:#fff">
        <div style="font-size:16px;font-weight:600;color:#0C1A57">Professores atuantes</div>
        <p style="font-size:14.5px;color:#5A6180;line-height:1.55;margin:8px 0 0">Quem palestra é quem atua diariamente na Administração Pública.</p>
      </div>
      <div style="border:1px solid #e5e5ea;border-radius:16px;padding:24px 26px;background:#fff">
        <div style="font-size:16px;font-weight:600;color:#0C1A57">Networking nacional</div>
        <p style="font-size:14.5px;color:#5A6180;line-height:1.55;margin:8px 0 0">Troque experiências com servidores e profissionais de todo o país.</p>
      </div>
      <div style="border:1px solid #e5e5ea;border-radius:16px;padding:24px 26px;background:#fff">
        <div style="font-size:16px;font-weight:600;color:#0C1A57">Certificado</div>
        <p style="font-size:14.5px;color:#5A6180;line-height:1.55;margin:8px 0 0">Certificado de participação individual para sua progressão funcional.</p>
      </div>
    </div>
  </div>
  ...
  <div style="max-width:1200px;margin:0 auto;padding:44px 30px 90px">
    <div style="position:relative;overflow:hidden;background:linear-gradient(120deg,#0A1442,#2A3F86 52%,#12235F);border:1px solid rgba(233,198,90,0.22);border-radius:16px;padding:52px 50px;text-align:center">
      <div style="font-size:12px;letter-spacing:2.4px;font-weight:700;color:#E9C65A">LEVE SUA EQUIPE</div>
      <h2 style="font-size:31px;color:#fff;margin:12px 0 0">Condições especiais para grupos e órgãos públicos</h2>
      <p style="font-size:17px;color:#B7C0DC;line-height:1.6;margin:14px auto 0;max-width:560px">Inscreva sua equipe com valores diferenciados. E se preferir um evento exclusivo para a sua instituição, conheça os treinamentos In Company.</p>
...
export function congressoHtml(c) {
  if (!c) return null;
  const waGrupo = `${WA}?text=${encodeURIComponent(`Olá, quero inscrever um grupo no evento ${escapeHtml(c.nome)}.`)}`;
  const chip = (v) =>
    `<span style="font-size:14px;font-weight:600;color:#fff;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.18);border-radius:999px;padding:9px 18px">${escapeHtml(v)}</span>`;
  const prog = c.prog
    .map((d, i) => `<details${i === 0 ? ' open' : ''} style="border:1px solid #e5e5ea;border-radius:5px;background:#fff">
        <summary style="cursor:pointer;padding:16px 34px 16px 18px;font-size:16px;font-weight:600;color:#0C1A57;display:flex;align-items:baseline;gap:12px"><span style="flex-shrink:0;font-size:12.5px;letter-spacing:1.4px;font-weight:700;color:#C9A227;text-transform:uppercase">${escapeHtml(d.dia)}</span>${escapeHtml(d.titulo)}</summary>
        <ul style="margin:0;padding:0 18px 16px 38px;display:flex;flex-direction:column;gap:7px">
          ${d.itens.map((it) => `<li style="font-size:14.5px;color:#5A6180;line-height:1.5">${escapeHtml(it)}</li>`).join('\n          ')}
        </ul>
      </details>`)
    .join('\n      ');
  return `<div>
  ...
  <div style="position:relative;background:radial-gradient(1200px 620px at 74% 34%, rgba(41,66,150,0.55), transparent 58%), linear-gradient(158deg,#0C1A57,#070E33)">
    <div style="max-width:1160px;margin:0 auto;padding:150px 30px 100px;text-align:center">
      ...
      <div style="margin-top:26px"><span style="display:inline-block;font-size:12px;letter-spacing:2.2px;font-weight:700;color:#E9C65A;background:rgba(233,198,90,0.12);border:1px solid rgba(233,198,90,0.32);border-radius:999px;padding:8px 18px">${escapeHtml(c.edicao.toUpperCase())}</span></div>
      <h1 style="font-weight:600;font-size:52px;line-height:1.06;color:#fff;margin:22px auto 0;max-width:820px">${escapeHtml(c.nome)}</h1>
      <p style="font-size:19px;line-height:1.6;color:#B7C0DC;margin:20px auto 0;max-width:620px">${escapeHtml(c.tagline)}</p>
      ...
      <div style="display:flex;gap:13px;justify-content:center;margin-top:36px;flex-wrap:wrap">
        <button type="button" data-ficha style="background:#E9C65A;color:#0A1442;border:0;border-radius:999px;padding:17px 34px;font-weight:700;font-size:16px;cursor:pointer;display:inline-flex;align-items:center;gap:8px" data-hv="gold">Garantir minha vaga <span>→</span></button>
  ...
  <div style="max-width:1160px;margin:0 auto;padding:56px 30px 0">
    <div style="background:#f5f5f7;border:1px solid #e5e5ea;border-radius:16px;padding:36px 38px;display:grid;grid-template-columns:1.1fr 0.9fr;gap:40px;align-items:center">
      <div>
        ...
        <p style="font-size:17px;color:#42496A;line-height:1.65;margin:14px 0 0">${escapeHtml(c.sobre)}</p>
      </div>
  ...
  <div style="max-width:1160px;margin:0 auto;padding:56px 30px 0">
    <div style="border:1px solid #e5e5ea;border-radius:16px;padding:32px 34px;display:grid;grid-template-columns:1fr 1fr;gap:40px;align-items:center">
      <div>
        ...
        <div style="font-size:22px;font-weight:600;color:#0C1A57;margin-top:10px">${escapeHtml(c.local_nome)}</div>
        <p style="font-size:15px;color:#5A6180;line-height:1.6;margin:10px 0 0">${escapeHtml(c.local_desc)}</p>
      </div>
      <div style="display:flex;flex-direction:column;gap:12px">
  ...
  <div style="max-width:1160px;margin:0 auto;padding:56px 30px 90px">
    <div style="background:linear-gradient(158deg,#0C1A57,#070E33);border-radius:16px;padding:52px 50px;text-align:center">
      <h2 style="font-size:31px;color:#fff;margin:0">Garanta sua vaga na ${escapeHtml(c.edicao)}</h2>
