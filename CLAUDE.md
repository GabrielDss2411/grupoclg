...
- **Conteúdo dinâmico vem do banco (Supabase)**: cursos e congressos moram nas tabelas `cursos`/`congressos` (Postgres, RLS: `anon` só vê `disponivel=true`). `lib/db.js` é a única porta de leitura (`getCursos`, `getCurso(slug)`, `getCongressos`, `getCongresso(slug)`), com cache por tag via `unstable_cache` (tags `cursos`/`congressos`, `revalidate: 3600`) sobre o client anon de `lib/supabase/server.js`. Publicar no painel chama `revalidateTag` e a mudança aparece em segundos; se o banco cair durante a regeneração, o Next serve a última versão gerada (stale) — o site nunca quebra por indisponibilidade do banco. Os mapas `CURSOS`/`CONGRESSOS` em `lib/cursos-data.js`/`lib/congressos-data.js` **deixaram de ser a fonte de dados**: hoje servem só de seed/fixture (testes, `scripts/seed`).
...
- **Rotas reais** (App Router): `/`, `/cursos`, `/congressos`, `/in-company`, `/sobre` + detalhes SSG/ISR em `/cursos/[slug]` e `/congressos/[slug]`. `generateStaticParams` lê do banco (`getCursos`/`getCongressos`); `export const dynamicParams = true` permite que um slug criado depois do build renderize on-demand; item ausente ou `disponivel=false` (RLS já filtra) → `notFound()`. Novas páginas = nova rota + novo export/entrada de dados + entrada em `ROUTES`.
- **Builders de HTML recebem objeto, não slug**: `lib/cursos-data.js` (`cursoHtml(curso)`) e `lib/congressos-data.js` (`congressoHtml(congresso)`, `congressosPageHtml(lista)`) recebem o objeto no formato das colunas do banco — nunca um slug string. Todo campo interpolado passa por `escapeHtml()` (`lib/escape.js`), porque o conteúdo agora é editável pelo cliente via painel. Countdown das LPs via `[data-countdown]`, recalculado no cliente em `Screen.jsx`.
